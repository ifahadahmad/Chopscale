...
//            setSliderThumb(thumbDrawable)
     ...
     * set changed image
     */
    fun setAfterImage(imageUri: Uri) {
        try{
            ClipDrawableProcessorTask<Uri>(this.after_image_view_id, seekbar_id, context, this).execute(imageUri)
        }catch (error:Throwable){
            Log.e("AFTERIMAGE",error.toString())
        }
